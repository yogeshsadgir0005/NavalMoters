...
const EmployeeDetail = () => {
      ...
      return (
          <tr className="bg-slate-50 border-b border-slate-200">
              <td colSpan="7" className="p-3 md:p-4">
                  <div className="bg-white border border-slate-200 rounded-lg p-3 md:p-4 max-w-3xl mx-auto shadow-inner overflow-x-auto">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                          <Icons.History /> Adjustment History
                      </p>
                      <table className="w-full text-left min-w-[350px] md:min-w-0">
                          <tbody className="divide-y divide-slate-100">
                              {adjustments.map((adj, i) => (
                                  <tr key={adj._id || adj.id || `${adj.type}-${adj.date}-${i}`}>
                                      <td className="py-1.5 md:py-1 text-xs font-bold text-slate-600 w-20 md:w-24">
                                      ...
                                      <td className="py-1.5 md:py-1 text-xs text-slate-700">{adj.reason}</td>
                                      <td className="py-1.5 md:py-1 text-[10px] text-slate-400 text-right whitespace-nowrap">
                                          {new Date(adj.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                                      </td>
                                      <td className={`py-1.5 md:py-1 text-xs font-bold text-right w-20 md:w-24 whitespace-nowrap ${adj.type === 'Incentive' ? 'text-emerald-600' : 'text-rose-600'}`}>
  ...
  if (!emp) return (
    <Layout>
        <div className="flex h-[80vh] items-center justify-center flex-col gap-3 p-4">
            {pageError ? (
                <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 md:px-6 py-4 rounded-lg flex flex-col items-center max-w-sm text-center">
                    <Icons.Alert />
  ...
  const TabButton = ({ id, label, Icon }) => (
    <button 
      ...
      className={`px-4 md:px-5 py-3 font-bold text-sm flex items-center gap-2 border-b-2 transition-all whitespace-nowrap shrink-0 ${
        activeTab === id 
  ...
  const InfoRow = ({ label, val }) => (
    <div className="flex flex-col border-b border-slate-50 py-3 last:border-0 hover:bg-slate-50/50 transition-colors px-2 rounded">
      ...
      <span className="font-semibold text-slate-800 text-sm break-words">{val || '—'}</span>
    </div>
  );

  return (
    <Layout>
      <div className="bg-white rounded-xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-slate-200 p-4 md:p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-3 md:gap-5">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-slate-100 to-slate-200 rounded-full flex items-center justify-center text-xl font-bold text-slate-500 overflow-hidden border-2 border-white shadow-sm ring-1 ring-slate-100 shrink-0">
                {emp.documents?.photo ? (
                    <img src={getFileUrl(emp.documents.photo)} className="w-full h-full object-cover" alt="Profile"/>
                ) : (
                    <span className="text-xl md:text-2xl">{emp.firstName?.[0]}</span>
                )}
            </div>
            <div className="min-w-0">
                <h1 className="text-lg md:text-xl font-bold text-slate-900 tracking-tight truncate">{emp.firstName} {emp.lastName}</h1>
                <div className="flex flex-wrap items-center gap-2 text-[10px] md:text-xs font-medium text-slate-500 mt-0.5 md:mt-1">
                    <span className="truncate">{emp.jobProfile?.name || 'No Role'}</span>
                    <span className="w-1 h-1 bg-slate-300 rounded-full shrink-0"></span>
                    <span className="truncate">{emp.department?.name || 'No Dept'}</span>
                </div>
                <div className="flex flex-wrap gap-1.5 md:gap-2 mt-2 md:mt-2.5">
                    <span className={`px-2 py-0.5 rounded text-[9px] md:text-[10px] font-bold uppercase tracking-wide border ${
                    ...
                    <span className="px-2 py-0.5 rounded text-[9px] md:text-[10px] font-bold bg-slate-50 text-slate-600 border border-slate-200 tracking-wide font-mono">
                        {emp.employeeCode}
                    </span>
                    {emp.status === 'Terminated' && (
                        <span className="px-2 py-0.5 rounded text-[9px] md:text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-100 uppercase tracking-wide">
                            Terminated
                        </span>
                    )}
                </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 md:gap-3 w-full md:w-auto mt-2 md:mt-0">
            {emp.status !== 'Terminated' && (
                <button 
                    onClick={() => setShowTerminateModal(true)} 
                    className="px-4 py-2 md:py-2 border border-rose-200 bg-white hover:bg-rose-50 text-rose-600 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-sm w-full md:w-auto"
                >
            ...
            <button 
                ...
                className="px-4 py-2 md:py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-sm w-full md:w-auto"
            >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-[500px] flex flex-col">
        ...
        <div className="p-4 md:p-8">
            {activeTab === 'self' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 md:gap-x-12 gap-y-6 md:gap-y-8 animate-in fade-in duration-300">
                    <div className="space-y-1">
                        <h3 className="font-bold text-slate-800 mb-3 md:mb-4 text-sm flex items-center gap-2">
            ...
            {activeTab === 'job' && (
                <div className="animate-in fade-in duration-300">
                    ...
                    {emp.lastJob?.company ? (
                        ...
                        <div className="p-8 md:p-12 text-center border-2 border-dashed border-slate-200 rounded-xl">
                            <p className="text-slate-400 text-xs md:text-sm font-medium italic">No employment history recorded.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {activeTab === 'financial' && (
                <div className="space-y-6 md:space-y-8 animate-in fade-in duration-300">
                    <div className="flex flex-col md:flex-row md:items-center justify-between bg-gradient-to-r from-slate-900 to-slate-800 p-4 md:p-6 rounded-xl shadow-lg text-white relative overflow-hidden">
                        <div className="relative z-10">
                            <span className="text-slate-300 text-[10px] font-bold uppercase tracking-widest">Current Base Salary</span>
                            <div className="flex items-baseline gap-2 mt-1">
                                <p className="text-3xl md:text-4xl font-bold tracking-tight">₹{emp.baseSalary?.toLocaleString()}</p>
                        ...
                        <button 
                            ...
                            className="mt-4 md:mt-0 w-full md:w-auto justify-center relative z-10 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-lg font-bold text-xs shadow-lg shadow-blue-900/50 transition-all active:scale-95 flex items-center gap-2"
                        >
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {emp.increments?.length > 0 ? [...emp.increments].reverse().map((inc, i) => (
                                        <tr key={i} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className={`px-4 py-3 text-xs font-bold whitespace-nowrap ${inc.type === 'Increment' ? 'text-emerald-600' : 'text-rose-600'}`}>
                                            ...
                                            <td className="px-4 py-3 text-xs text-slate-500 italic break-words min-w-[150px]">{inc.reason}</td>
                                        </tr>
                                    )) : (
                                ...
                                {salaryHistory.length > 0 ? salaryHistory.map((sal) => {
                                    ...
                                    return (
                                        <div key={sal._id} className="contents">
                                            ...
                                            >
                                                <td className="px-4 py-3 text-sm font-bold text-blue-700 whitespace-nowrap">{sal.month}</td>
                                                <td className="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">{new Date(sal.updatedAt).toLocaleDateString('en-IN')}</td>
                                                <td className="px-4 py-3 text-center">
                                                    <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase whitespace-nowrap">{sal.presentDays} Days</span>
                                                </td>
                                                <td className={`px-4 py-3 text-right text-xs font-bold whitespace-nowrap ${sal.incentives >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                    {sal.incentives > 0 ? '+' : ''}₹{sal.incentives.toLocaleString()}
                                                </td>
                                                <td className="px-4 py-3 text-right text-sm font-bold text-slate-900 font-mono whitespace-nowrap">₹{sal.netPay.toLocaleString()}</td>
                                                <td className="px-4 py-3 text-center">
            ...
            {activeTab === 'docs' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 animate-in fade-in duration-300">
                        ...
                        if (Array.isArray(val)) {
                            ...
                            return val.map((filename, index) => (
                                <a 
                                    ...
                                    rel="noreferrer" 
                                    key={`${key}-${index}`} 
                                    className="group flex flex-col items-center justify-center p-4 md:p-6 border border-slate-200 rounded-xl hover:border-blue-300 hover:bg-blue-50/30 transition-all cursor-pointer bg-white shadow-sm hover:shadow-md"
                                >
                                    <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-2 md:mb-3 group-hover:scale-110 transition-transform shrink-0">
                                        <Icons.File />
                                    </div>
                                    <div className="text-[11px] md:text-xs font-bold uppercase text-slate-600 group-hover:text-blue-700 text-center break-words w-full px-2">
                                        {key.replace(/([A-Z])/g, ' $1')} 
                                        <span className="ml-1 text-slate-400 opacity-70">#{index + 1}</span>
                                    </div>
                                    <div className="text-[9px] md:text-[10px] text-slate-400 mt-1 font-medium group-hover:text-blue-500">Click to View</div>
                                </a>
                            ));
                        }
                    ...
                    {!hasAnyDocs(emp.documents) && (
                        <div className="col-span-full p-8 md:p-10 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-400">
                            <Icons.Folder />
                            <span className="text-xs md:text-sm font-medium mt-2 text-center">No documents uploaded</span>
                        </div>
                    )}
                </div>
            )}
      ...
      {showIncModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden border border-slate-100">
                <div className="px-4 md:px-6 py-3 md:py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                        <Icons.Banknote /> Update Salary Structure
                    </h3>
                    <button onClick={() => { setShowIncModal(false); setModalError(null); }} className="text-slate-400 hover:text-rose-500 transition-colors p-1"><Icons.X /></button>
                </div>
                
                <form onSubmit={handleIncrement} className="p-4 md:p-6 space-y-4 md:space-y-5">
                    {modalError && (
                        <div className="bg-rose-50 border border-rose-200 text-rose-700 px-3 md:px-4 py-2 md:py-3 rounded-lg text-xs md:text-sm flex items-start gap-2 md:gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] md:text-xs mt-0.5">{modalError}</p>
                            </div>
                        </div>
                    )}

                    <div>
                        <label className="block text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 md:mb-2">Action Type</label>
                        <div className="grid grid-cols-2 gap-2 md:gap-3">
                            {['Increment', 'Decrement'].map(type => (
                                <button
                                    ...
                                    className={`py-2 md:py-2.5 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-wider border transition-all flex items-center justify-center gap-1 md:gap-2 ${
                                        incForm.type === type
                    ...
                    <div>
                        <label className="block text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1 md:mb-1.5">Adjustment Amount (₹)</label>
                        <input 
                            type="number" 
                            required
                            className="w-full border border-slate-200 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-xs md:text-sm font-semibold focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                    ...
                    <div>
                        <label className="block text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1 md:mb-1.5">Reason / Justification</label>
                        <textarea 
                            required
                            rows="3"
                            className="w-full border border-slate-200 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-xs md:text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-all"
                    ...
                    <div className="pt-2">
                        <button 
                            type="submit" 
                            disabled={updating}
                            className={`w-full py-2.5 md:py-3 rounded-lg text-white text-xs md:text-sm font-bold uppercase tracking-wider shadow-lg transition-all active:scale-95 ${
              ...
              <div className="bg-white w-full max-w-md rounded-xl shadow-2xl border border-slate-100 overflow-hidden">
                  <div className="px-4 md:px-6 py-3 md:py-4 border-b border-slate-100 flex justify-between items-center bg-rose-50/30">
                      <div className="flex items-center gap-2 text-rose-700">
                          <Icons.Alert />
                          <h3 className="font-bold text-rose-900 text-sm md:text-base">Terminate Employee</h3>
                      </div>
                      <button onClick={() => setShowTerminateModal(false)} className="text-slate-400 hover:text-slate-600 p-1"><Icons.XCircle /></button>
                  </div>
                  <div className="p-4 md:p-6">
                      <p className="text-xs md:text-sm text-slate-600 mb-4 leading-relaxed">
                          Are you sure you want to terminate <strong className="text-slate-800 break-words">{emp.firstName} {emp.lastName}</strong>?
                      </p>
                      <ul className="text-[10px] md:text-xs text-slate-500 space-y-2 mb-5 md:mb-6 bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                      ...
                      <div className="flex flex-col-reverse sm:flex-row gap-2 md:gap-3 justify-end">
                          <button onClick={() => setShowTerminateModal(false)} className="w-full sm:w-auto px-4 py-2 md:px-5 md:py-2.5 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 font-bold text-slate-600 text-xs md:text-sm transition-all text-center">
                              Cancel
                          </button>
                          <button onClick={handleTerminate} disabled={terminating} className="w-full sm:w-auto px-4 py-2 md:px-5 md:py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-bold text-xs md:text-sm shadow-md shadow-rose-200 transition-all flex items-center justify-center gap-2">
